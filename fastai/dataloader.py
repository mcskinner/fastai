...
class DataLoader(object):
    """Split a dataset into batches, and then be iterable over those batches."""
    ...
    def __iter__(self):
        if self.num_workers==0:
            for batch in map(self.get_batch, iter(self.batch_sampler)):
                yield get_tensor_(batch, self.pin_memory)
        else:
    ...
    def get_batch(self, indices):
        res = self.np_collate_([self.dataset[i] for i in indices])
        if self.transpose:   res[0] = res[0].T
        if self.transpose_y: res[1] = res[1].T
        return res

    def stack_jagged_(self, b):
    ...
    def np_collate_(self, batch):
        b = batch[0]
        if isinstance(b, (np.ndarray, np.generic)): return self.stack_jagged_(batch)
            ...
            return {key: self.np_collate_([d[key] for d in batch]) for key in b}
        elif isinstance(b, collections.Sequence):
            return [self.np_collate_(samples) for samples in zip(*batch)]
...
def get_tensor_(batch, pin):
    if isinstance(batch, (np.ndarray, np.generic)):
        ...
        return {k: get_tensor_(sample, pin) for k, sample in batch.items()}
    elif isinstance(batch, collections.Sequence):
        return [get_tensor_(sample, pin) for sample in batch]
